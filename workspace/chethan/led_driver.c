...
#define IOCTL_RED_LED_ON_PARAM _IOW(IOCTL_MAGIC, 2, int)
...
static long chethan_unlocked_ioctl (struct file *file, unsigned int command, unsigned long){
	switch(command){
		case IOCTL_RED_LED_ON: 
			printk(KERN_INFO "Without param red led\n");
			break;
		case IOCTL_RED_LED_ON_PARAM:
			printk(KERN_INFO "With param red led\n");
			break;
		default:
